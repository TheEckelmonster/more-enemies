...
local function set_game(event, __game, __storage)
    ...
    --         Force_Funcs[name].is_friend = force.is_friend
    --     else
    --         Forces[name] = nil
    --     end
    -- end
...
-- log(serpent.block(metrics_filter))
...
function metrics_controller.on_post_entity_died(event)
    ...
    local spawner_loss = 0

    if (proto.is_building and (was_killed_by_NOT_player or was_killed_by_enemy)) then
        player_infrastructure_loss = 1
    elseif (types[proto.type] and (was_killed_by_NOT_enemy or was_killed_by_player)) then
        enemy_combatant_loss = 1
    elseif (proto.type == UNIT_SPAWNER) then
        log("is_spawner")
        spawner_loss = 1
    else
        return
    ...
    evolution_factors = evolution_factors or set_game() and evolution_factors

    force_funcs = force_funcs or set_game() and force_funcs
    evolution_factors[force_name] = evolution_factors[force_name] or force_funcs[force_name].get_evolution_factor(surface_name)
    -- weight = weight ^ (evolution_factors[force_name] or 0)
    weight = math_pow(weight, evolution_factors[force_name] or 0)

    -- c_meta.entity_count = chunk.entity_count or 0
    -- c_meta.spawner_count = chunk.spawner_count or 0
    c_meta.entity_count = (c_meta.entity_count or 0) - player_infrastructure_loss
    c_meta.entity_delta = 0 - player_infrastructure_loss
    c_meta.spawner_count = (c_meta.spawner_count or 0)
    c_meta.spawner_delta = 0 - spawner_loss

    c_meta.witnessed = 1
    c_meta.witnessed_tick = event.tick

    c_meta.witnessed_entity_delta = (c_meta.entity_count or 0) - (c_meta.witnessed_entity_count or 0)
    c_meta.witnessed_entity_count = c_meta.entity_count or 0

    -- c_meta.vision_factor = math_min(1.0, math_max(0.001, c_meta.witnessed_entity_count) / math_max(1, c_meta.entity_count))

    c_meta.death_weight = c_meta.death_weight + weight
    c_meta.prev_death_tick = c_meta.last_death_tick or 0
    c_meta.last_death_tick = event.tick

    c_meta.player_deaths = (c_meta.player_deaths or 0) + player_infrastructure_loss
    c_meta.enemy_deaths = (c_meta.enemy_deaths or 0) + enemy_combatant_loss

    c_meta.total_player_deaths = (c_meta.total_player_deaths or 0) + player_infrastructure_loss
    c_meta.total_enemy_deaths = (c_meta.total_enemy_deaths or 0) + enemy_combatant_loss + spawner_loss

    c_meta.total_deaths = c_meta.total_deaths + player_infrastructure_loss + enemy_combatant_loss + spawner_loss
...
function metrics_controller.on_entity_died(event)
    stats_data = stats_data or set_game() and stats_data
    stats_data.current.total = (stats_data.current.total or 0) + 1
    if (not event) then return end
    -- log(serpent.block(event))
    ...
    unit_groups = unit_groups or set_game() and unit_groups
    unit_groups.unit_nums = unit_groups.unit_nums or {}
    local source = unit_groups.unit_nums[unit_number] and GROUP or SPAWNED
    unit_groups.unit_nums[unit_number or 0] = nil
    ...
    local killing_force_name = event.force and event.force.valid and event.force.name or nil

    force_funcs = force_funcs or set_game() and force_funcs

    local killed_by_PLAYER = killing_force_name and killing_force_name == PLAYER
    local killed_by_ENEMY = killing_force_name and killing_force_name == ENEMY
    local killed_by_player_friend = killing_force_name and (killing_force_name == PLAYER or force_funcs and force_funcs[killing_force_name].get_friend(PLAYER)) or false
    local killed_by_player_enemy = killing_force_name and force_funcs and force_funcs[killing_force_name].is_enemy(PLAYER) or false
    local killed_by_neutral = killing_force_name == NEUTRAL

    local player_infrastructure_loss = 0
    local enemy_combatant_loss = 0
    local spawner_loss = 0

    if ((force_name == ENEMY or force_name ~= PLAYER) and entity_type == UNIT_SPAWNER) then
        spawner_loss = 1
    elseif ((force_name == ENEMY or types[entity_type] and force_name ~= PLAYER) and (killed_by_PLAYER or killed_by_player_friend or killed_by_neutral)) then
        enemy_combatant_loss = 1
    elseif ((force_name == PLAYER or not types[entity_type]) and (killed_by_ENEMY or killed_by_player_enemy or killed_by_neutral or entity_type == LANDMINE)) then
        player_infrastructure_loss = 1
    else
        return
    end
    ...
    local x = math_floor(entity.position.x / CHUNK_SIZE)
    local y = math_floor(entity.position.y / CHUNK_SIZE)
    local xy = pack_coordinates(x, y)
    ...
    --[[
        -> Death weight_formula
        4 + (1 / (1 + (total_enemy_deaths / 4) ^ 0.5))*(128 - (128 / (1 + ((((total_enemy_deaths / 4) ^ 2) / 12) / 32))))
    ]]

    evolution_factors = evolution_factors or set_game() and evolution_factors

    if (selected_style == OMNI_MIND) then
        force_funcs = force_funcs or set_game() and force_funcs
        evolution_factors[force_name] = evolution_factors[force_name] or force_funcs[force_name].get_evolution_factor(surface_name)
        weight = weight ^ (evolution_factors[force_name] or 0)
    end
    ...
    -- c_meta.spawner_count = chunk.spawner_count or 0
    c_meta.entity_count  = (c_meta.entity_count or 0)-- - player_infrastructure_loss
    c_meta.spawner_count = (c_meta.spawner_count or 0)-- - spawner_loss

    c_meta.entity_delta  = (c_meta.entity_delta or 0) - player_infrastructure_loss
    c_meta.spawner_delta = (c_meta.spawner_delta or 0) - spawner_loss
...
Event_Handler:register_event({
    ...
    filter = metrics_filter,
    source_name = "metrics_controller.on_entity_died",
    func_name = "metrics_controller.on_entity_died",
    func = metrics_controller.on_entity_died,
})
