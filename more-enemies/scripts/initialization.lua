...
function locals.initialize(from_scratch, maintain_data, maintain_existing_peace)
    ...
    for _, planet in pairs(Constants.DEFAULTS.planets) do
        ...
        if (not more_enemies_data.attack_group[planet.string_val]) then more_enemies_data.attack_group[planet.string_val] =
            Attack_Goup_Data:new({ surface_name = planet.string_val }, { include_meta_data }) end

        if (not more_enemies_data.overmind) then more_enemies_data.overmind = {} end
        if (not more_enemies_data.overmind[planet.string_val]) then
            more_enemies_data.overmind[planet.string_val] = Overmind_Data:new({ surface_name = planet.string_val },
                { include_meta_data })
        else
            more_enemies_data.overmind[planet.string_val] = Overmind_Data:reinit(more_enemies_data.overmind
            [planet.string_val])
        end
        if (game) then more_enemies_data.overmind[planet.string_val].surface = game.get_surface(planet.string_val) end
...
function locals.migrate(data)
    ...
    for _, planet in pairs(Constants.DEFAULTS.planets) do
        if (game and game.surfaces and game.surfaces[planet.string_val] and game.surfaces[planet.string_val].valid) then
            ...
            if (storage_old.more_enemies.overmind and storage_old.more_enemies.overmind[planet.string_val]) then
                local overmind_data_old = storage_old.more_enemies.overmind[planet.string_val]

                -- if (type(overmind_data_old) == "table" and overmind_data_old.surface and overmind_data_old.surface.valid) then
                if (type(overmind_data_old) == "table" and overmind_data_old.surface and overmind_data_old.surface == planet.string_val) then
                    Overmind_Repository.update_overmind_data(overmind_data_old)
                end

                storage_old.more_enemies.overmind[planet.string_val] = nil
            end
        end
    end
end
