...
function planet_service.on_surface_created(event)
    ...
    local surface = (game or set_game()).get_surface(event.surface_index)
    if (not surface or not surface.valid) then return end

    local surface_name = surface.name

    -- planetary_surfaces[surface_name] = surface
    -- planetary_surfaces.list[surface.index] = surface_name
...
function planet_service.on_surface_deleted(event)
    ...
    -- local surface_name = planetary_surfaces.list[event.surface_index]
...
function planet_service.on_surface_renamed(event)
    ...
    -- planetary_surfaces[new_name] = planetary_surfaces[old_name]
    -- planetary_surfaces[old_name] = nil
    -- planetary_surfaces.list[event.surface_index] = new_name
