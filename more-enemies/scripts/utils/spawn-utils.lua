...
function spawn_utils.clone_entity(entity, unit_type, surface_name, unit_clone_setting, unit_group_clone_setting, tick)
    ...
    if (not surface or not surface.valid) then
        ...
        Surfaces.list[entity.surface.index] = surface_name
    end
    ...
    -- if (types[params.unit_type]) then
    --     loop_len = math_floor(((clone_settings[params.unit_type] or 0) + selected_difficulty.value) * (use_evolution_factor[surface_name] and clone_settings.evolution_multiplier or selected_difficulty.fallback_evolution_multiplier or 1)
    --              + (((clone_settings[params.unit_type] or 1) * selected_difficulty.value) * (use_evolution_factor[surface_name] and clone_settings.evolution_multiplier or selected_difficulty.fallback_evolution_multiplier or 1)) + 1)
    -- else
    --     loop_len = math_floor(1.5 * (selected_difficulty.value * selected_difficulty.value) * (use_evolution_factor[surface_name] and clone_settings.evolution_multiplier or selected_difficulty.fallback_evolution_multiplier or 1) + 1)
    -- end
